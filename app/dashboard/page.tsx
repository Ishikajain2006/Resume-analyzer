...
import {
  ...
  ArrowRight, Zap, ChevronDown, Play, Database, Lock, Sparkles,
} from "lucide-react";
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const [res] = await Promise.all([
        ...
        new Promise(r => setTimeout(r, 2000)) // Smooth terminal loading
      ]);
                ...
                {!isSignedIn && isLoaded && (
                  <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 13, color: "#fff" }}>Ready for Analysis</div>
                        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>AI ATS diagnostics and interview simulations are fully unlocked. Sign in anytime to sync history.</div>
                      </div>
                    </div>
                    <button
                      onClick={() => { sounds.click(); openSignIn(); }}
                      className="btn-ghost"
                      style={{ height: 32, fontSize: 12, padding: "0 14px", border: "1px solid rgba(168,85,247,0.4)", color: "#d8b4fe" }}
                    >
                      Sign In (Optional) →
                    </button>
                  </div>
      ...
      <AnimatePresence>
        {isInspectorOpen && resumeText && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }}
            style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(12px)" }}
          ...
          >
            <motion.div
              ...
              className="panel"
              style={{ width: "100%", maxWidth: 800, maxHeight: "85vh", display: "flex", flexDirection: "column", overflow: "hidden", background: "#0c0714" }}
        ...
        {isMatchOpen && resumeText && atsAnalysis && (
          ...
          >
            <motion.div
              ...
              transition={{ duration: 0.2, ease: "easeOut" }}
              className="panel"
              style={{ width: "100%", maxWidth: 1000, maxHeight: "85vh", display: "flex", flexDirection: "column", overflow: "hidden", background: "#0c0714" }}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="terminal-chrome" style={{ padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
